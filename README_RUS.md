...
### Админ-панель (`/admin`)
Отдельный контур (своя таблица `admin_users`, cookie-сессия в Redis) для поддержки: список пользователей с поиском и пагинацией и карточка пользователя; бан и разбан (действует со следующего запроса); принудительный разлогин со всех устройств; удаление аккаунта с анонимизацией. Необратимые действия пишутся в `admin_audit_log` одной транзакцией с самой мутацией. Мобильный клиент может удалить свой аккаунт через `DELETE /v1/account` (требование App Store / Google Play): аккаунт удаляется мягко с анонимизацией (email обнуляется, пароль и соцпривязки удаляются, refresh-токены отзываются), а сообщения остаются в истории чатов от имени «Удалённый пользователь».
